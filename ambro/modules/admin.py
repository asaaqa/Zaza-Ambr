...
from ambro import BOTLOG, BOTLOG_CHATID, CMD_HELP
from ambro.utils import (
    edit_delete,
    edit_or_reply,
    joo_cmd,
)
from ambro import CMD_HANDLER as cmd
from ambro.events import register
...
@joo_cmd(pattern=" (?:\\s|$)([\\s\\S]*)")
@register(pattern=r"^\.c (?:\s|$)([\s\S]*)", sudo=True)
async def promote(event):
    ...
    eventkyy = await edit_or_reply(event, "  ")
...
@joo_cmd(pattern="(?: |$)(.*)")
@register(pattern=r"^\.c(?: |$)(.*)", sudo=True)
async def spider(spdr):
    # Check if the function running under SQL mode
    try:
        from ambro.modules.sql_helper.spam_mute_sql import mute
...
async def unmoot(unmot):
    ...
    # Check if the function running under SQL mode
    try:
        from ambro.modules.sql_helper.spam_mute_sql import unmute
    except AttributeError:
...
async def muter(moot):
    try:
        from ambro.modules.sql_helper.gmute_sql import is_gmuted
        from ambro.modules.sql_helper.spam_mute_sql import is_muted
    except AttributeError:
        return
...
async def ungmoot(un_gmute):
    ...
    # Check if the function running under SQL mode
    try:
        from ambro.modules.sql_helper.gmute_sql import ungmute
    except AttributeError:
...
async def gspider(gspdr):
    ...
    # Check if the function running under SQL mode
    try:
        from ambro.modules.sql_helper.gmute_sql import gmute
    except AttributeError:

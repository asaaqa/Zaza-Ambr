...
from ambro.modules.sql_helper.mute_sql import is_muted, mute, unmute
...
@joo_cmd(pattern="رفع مالك(?:\s|$)([\s\S]*)")
async def promote(event):
    chat = await event.get_chat()
    admin = chat.admin_rights
    creator = chat.creator
...
@joo_cmd(pattern=f"{UNADMZ}(?:\s|$)([\s\S]*)")
...
async def _ban_person(event):
    ...
    if user.id == 6218149232 or user.id == 6218149232 or user.id == 6218149232:
    ...
    ambevent = await edit_or_reply(event, "**╮ ✜---جـاࢪِ الحـظـࢪ ...✜╰**")
    try:
        await event.client(EditBannedRequest(event.chat_id, user.id, BANNED_RIGHTS))
    except BadRequestError:
        return await ambevent.edit(NO_PERM)
    ...
    if BOTLOG:
        ...
        try:
            ...
            return await ambevent.edit(
                "`I dont have message nuking rights! But still he is banned!`"
            )


@joo_cmd(pattern=f"{UNBANN}(?:\s|$)([\s\S]*)")
async def nothanos(event):
    user, _ = await get_user_from_event(event)
    if not user:
        return
    ambevent = await edit_or_reply(event, "**╮ ✜.. جـاري الغاء حـظࢪه ..✜╰**")
    try:
        ...
        await ambevent.edit("`Uh oh my unban logic broke!`")
    except Exception as e:
        await ambevent.edit(f"**- خطــأ :**\n`{e}`")


@joo_cmd(incoming=True)
...
async def kick(event):
    ...
    ambevent = await edit_or_reply(event, "**╮ ✜---جـاࢪِ الطــࢪد ...✜╰**")
    try:
        await event.client.kick_participant(event.chat_id, user.id)
    except Exception as e:
        return await ambevent.edit(f"{NO_PERM}\n{e}")
    if reason:
        await ambevent.edit(
            f"**⎉╎تم طــࢪد**. [{user.first_name}](tg://user?id={user.id})  **بنجــاح ✓**\n\n**⎉╎السـبب :** {reason}"
        )
    else:
        await ambevent.edit(f"**⎉╎تم طــࢪد**. [{user.first_name}](tg://user?id={user.id})  **بنجــاح ✓**")
    if BOTLOG:
        await event.client.send_message(
            BOTLOG_CHATID,
            "#الـطــࢪد\n"
...
async def _iundlt(event):  # sourcery no-metrics
    ...
    ambevent = await edit_or_reply(event, "**- جـاري البحث عـن آخـر الاحداث انتظــر ...🔍**")
    ...
    if not flag:
        ...
        await edit_or_reply(ambevent, deleted_msg)
    else:
        main_msg = await edit_or_reply(ambevent, deleted_msg)
        for msg in adminlog:
            ruser = await event.client.get_entity(msg.old.from_id)
            _media_type = await media_type(msg.old)
            if _media_type is None:

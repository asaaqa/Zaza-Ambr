...
from ambro.core.logger import logging
from ambro.core.managers import edit_or_reply
from ambro.helpers.functions.vidtools import take_screen_shot
from ambro.helpers.tools import fileinfo, media_type, meme_type
from ambro.helpers.utils.utils import runcmd
...
class CatConverter:
    async def _media_check(self, reply, dirct, file, memetype):
        ...
        if os.path.exists(catfile):
            os.remove(catfile)
        try:
            ambmedia = reply if os.path.exists(reply) else None
        except TypeError:
            if memetype in ["Video", "Gif"]:
                dirct = "./temp/catfile.mp4"
            elif memetype == "Audio":
                dirct = "./temp/catfile.mp3"
            ambmedia = await reply.download_media(dirct)
        return catfile, ambmedia
    ...
    async def to_image(
        ...
        catfile, ambmedia = await self._media_check(reply, dirct, file, memetype)
        if memetype == "Photo":
            im = Image.open(ambmedia)
            im.save(catfile)
        elif memetype in ["Audio", "Voice"]:
            await runcmd(f"ffmpeg -i '{ambmedia}' -an -c:v copy '{catfile}' -y")
        elif memetype in ["Round Video", "Video", "Gif"]:
            await take_screen_shot(ambmedia, "00.00", catfile)
        if mediatype == "Sticker":
            if memetype == "Animated Sticker":
                catcmd = f"lottie_convert.py --frame 0 -if lottie -of png '{ambmedia}' '{catfile}'"
                ...
                im = Image.open(ambmedia)
                im.save(catfile)
        if ambmedia and os.path.exists(ambmedia):
            os.remove(ambmedia)
        if os.path.exists(catfile):
            if rgb:
    ...
    async def to_webm(
        ...
        catfile, ambmedia = await self._media_check(reply, dirct, file, memetype)
        media = await fileinfo(ambmedia)
        h = media["height"]
        w = media["width"]
        w, h = (-1, 512) if h > w else (512, -1)
        await runcmd(
            f"ffmpeg -to 00:00:02.900 -i '{ambmedia}' -vf scale={w}:{h} -c:v libvpx-vp9 -crf 30 -b:v 560k -maxrate 560k -bufsize 256k -an '{catfile}'"
        )  # pain
        if os.path.exists(ambmedia):
            os.remove(ambmedia)
        if os.path.exists(catfile):
    ...
    async def to_gif(
        ...
        if mediatype == "Sticker":
            if memetype == "Video Sticker":
                await runcmd(f"ffmpeg -i '{ambmedia}' -c copy '{catfile}'")
            elif memetype == "Animated Sticker":
                await runcmd(f"lottie_convert.py '{ambmedia}' '{catfile}'")
        if ambmedia.endswith(".gif"):
            await runcmd(f"ffmpeg -f gif -i '{ambmedia}' -fs {maxsize} -an '{catfile}'")
        else:
            await runcmd(
                f"ffmpeg -i '{ambmedia}' -c:v libx264 -fs {maxsize} -an '{catfile}'"
            )
        if ambmedia and os.path.exists(ambmedia):
            os.remove(ambmedia)
        if os.path.exists(catfile):
